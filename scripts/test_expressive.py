...
def main(mode, checkpoint_path):
    ...
    def load_dataset(path):
        try:
            ...
            if len(dataset) == 0:
                logging.warning("Dataset is empty! Trying to rebuild cache...")
                # Try to force rebuild the cache - handle directory case
                cache_path = path + '_cache'
                if os.path.isdir(cache_path):
                    logging.info(f"Cache is a directory: {cache_path}")
                    # You might need to implement a way to clear the directory
                    # or pass a flag to your dataset to rebuild the cache
                else:
                    try:
                        os.remove(cache_path)
                    except Exception as e:
                        logging.error(f"Failed to remove cache: {e}")
                        
                # Try to rebuild with a flag
                dataset = SpeechMotionDataset(path,
                                             n_poses=args.n_poses,
                                             subdivision_stride=args.subdivision_stride,
                                             pose_resampling_fps=args.motion_resampling_framerate,
    ...
    if mode == 'eval':
        ...
        # Skip the dataset loading and go directly to LMDB approach
        logging.info("Using direct LMDB approach for short mode...")
        
        # Direct LMDB approach
        ...
        with lmdb_env.begin(write=False) as txn:
            ...
            for key in keys:
                if n_saved >= 5:  # Save 5 videos
                    break
                    
                try:
                    # Try using pyarrow to deserialize
                    buf = txn.get(key)
                    try:
                        video = pyarrow.deserialize(buf)
                    except Exception as e:
                        logging.error(f"PyArrow deserialization failed: {e}")
                        continue
                    ...
                    # Process the clip similar to the long mode
                    ...
                    for word_info in clip_words:
                        word_seq.append([word_info[0], word_info[1] - clip_time[0], word_info[2] - clip_time[0]])
                    
                    # Generate gestures
                    out_dir_vec = generate_gestures(args, diffusion, lang_model, clip_audio, word_seq, pose_dim,
                                                   seed_seq=target_dir_vec[0:args.n_pre_poses], fade_out=False)
                    
                    # Create video
                    sentence = ' '.join([word[0] for word in word_seq])
                    aux_str = '({}, time: {}-{})'.format(vid, 
                    ...
                    create_video_and_save(
                        save_path, n_saved, 'short',
                        target_dir_vec, out_dir_vec, mean_data,
                        sentence, audio=clip_audio, aux_str=aux_str)
